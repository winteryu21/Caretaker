...
- **담당자 매핑:** Team A → 유민서(winteryu21), Team B → 김도경(kjskjk7935)
...
- 참고자료: `game-design.md` §4~§6, `game-design.md` Step 4, 7, 9, 10
...
- 배경: game-design.md §6에 구간별 장애물이 초안 수준. 실제 레벨 제작 전 확정 필요
- 상세 작업 내용:
  - 3구간 각각의 장애물 종류, 개인/인과/역인과 분류
  - 과거 프로토타입 로봇 / 미래 진보한 로봇의 추격 파라미터
  - 최종 봉쇄문 동시 조작 판정 방식
  - 타임아웃 존재 여부 및 값
- 참고자료: `game-design.md` §6, `game-design.md` Step 11
...
- 배경: game-design.md §2에 P1 4개, P2 5개, P3 4개 = 13개 규칙이 정의됨. 일부는 구체적 조건이 TBD
- 상세 작업 내용:
...
- 상세 작업 내용:
  ...
  - 하이라이트 아웃라인 on/off
  - objectId 필드 (OBJ_P1_SIGN 등 game-design.md §7의 ID와 매핑)
  - Team B가 Inspector에서 설정 가능한 형태
- 참고자료: `DSD.md` §3.4, `game-design.md` §7
...
- 배경: 카드키→자료실 리더, 드라이버→환풍구 등 game-design.md의 아이템 사용 흐름
- 상세 작업 내용:
  - 선택한 아이템을 InteractableObject에 우클릭 → requiredItemId 매칭 검증
  - 성공 시 CausalTrigger 발동 또는 상태 변경
  - 실패 시 "사용할 수 없습니다" 피드백
- 참고자료: `game-design.md` §4~§6 상세 단계의 UseItemOnTarget
...
- **Label:** Team A, E2-Causality
- **Description:**
- 목표: Phase 3의 인과 규칙 4개를 ScriptableObject 에셋으로 작성한다
- 배경: C동 탈출 시퀀스에서 과거 조작 → 미래 경로 개방에 필요한 인과 데이터
- 상세 작업 내용:
  - CR_P3_BRIDGE_DROP: 다리 레버 조작 → 구멍 위에 다리 생성
  - CR_P3_DOOR_UNLOCK: 비상문 잠금 해제 → 문 열림
  - CR_P3_PLATFORM_LOWER: 플랫폼 장치 조작 → 플랫폼 하강
  - CR_P3_TOOL_PLACE: 도구 설치 → 장애물 제거 (TBD 구체화 후)
- 참고자료: `game-design.md` §2 Phase 3
- 수용 기준: 4개 SO 에셋 생성, Inspector에서 모든 필드 입력 완료
...
- 상세 작업 내용:
  ...
  - P1: 로비, 변전실 가는 길, 변전실 앞 복도, 변전실, 보안실 앞 복도, 보안실
  - P2: 연구소장실, 2층 복도, 구름다리, 자료실, 실험실, 샘플 저장고
  - P3: C동 3구간
...
- **Label:** Team A, E3-Room
- **Description:**
- 목표: Persistent 씬 + Phase별 씬 Additive 로드 구조를 구축한다
- 배경: 과거/미래는 별도 씬(별도 맵). 각 플레이어는 자기 시간대의 Phase 씬만 로드
- 상세 작업 내용:
  - Persistent.unity: NetworkManager, 공통 매니저, UI Canvas
  - Phase별 씬 6개: Phase1_Past, Phase1_Future, Phase2_Past, Phase2_Future, Phase3_Past, Phase3_Future
  - SceneLoader 유틸리티: Additive 로드/언로드 API
  - Netcode SceneManagement 연동
- 참고자료: `docs/technical/architecture.md`, `DSD.md` §3.2
...
- **Label:** Team A, E3-Room
- **Description:**
- 목표: 각 플레이어에게 자기 시간대 씬만 로드하여 정보 격리를 보장한다
- 배경: 과거 플레이어는 미래 환경을 볼 수 없고, 미래 플레이어는 과거 환경을 볼 수 없음
- 상세 작업 내용:
  - Host는 역할(TimelineRole)에 따라 Phase{N}_Past 또는 Phase{N}_Future 씬을 Additive 로드
  - Client도 동일하게 자기 시간대 씬만 로드
  - 인과 이벤트는 네트워크 RPC로 전파 (CausalityService 연동)
- 참고자료: `DSD.md` §3.5
...
- 상세 작업 내용:
  ...
  - game-design.md §4~§6의 상세 단계와 매핑
- 참고자료: `game-design.md`, `game-design.md` §4~§6
...
- **Label:** Team B, E5-Level
- **Description:**
- 목표: Phase 1의 룸을 Unity 타일맵으로 화이트박스 구축한다
...
- **Label:** Team B, E5-Level
- **Description:**
- 목표: Phase 2의 룸을 Unity 타일맵으로 화이트박스 구축한다
...
- 배경: game-flow Step 11, game-design.md §6. A/B동과 별개인 독립 공간
- 상세 작업 내용:
...
- **Label:** Team A, E6-Network
- **Description:**
- 목표: 2인 간 PTT(Push-to-Talk) 기반 음성 소통 시스템을 구현한다
- 배경: game-design.md의 모든 📻 무전 교환이 이 시스템에 의존. 시스템은 송신권 관리만 담당하며, 정보 전달 여부는 추적하지 않는다 (순수 음성)
- 상세 작업 내용:
  - PTT 키 바인딩 (기본: T키)
  - 송신/수신 상태 관리 (한 번에 한 명만 송신)
  - Unity 음성 채팅 또는 외부 솔루션 연동 (TBD)
  - HUD 무전기 상태 표시 연동
- 참고자료: `DSD.md` §3.6, `game-design.md` §1 범례 📻
- 수용 기준: PTT 키 누르면 상대에게 음성 전달, 송신 상태가 HUD에 표시
...
- 배경: game-design.md §6 판정 — 한쪽이라도 포획되면 공동 실패 → 구간 시작점 복귀
- 상세 작업 내용:
  - 스냅샷: 플레이어 위치, 인벤토리, 인과 완료 상태
  - 자동 저장 시점: Phase 전환, Major 완료
  - 공동 실패 시 마지막 체크포인트 복원
- 참고자료: `DSD.md` §3.9, `game-design.md` §6 판정
